...
// Loads a real page twice: once on a fresh connection (cold) and once warm.
export async function probeBrowsing({ workload, proxy = null, timeoutMs = 15000, assetLimit = 6, logger = null }) {
  ...
  try {
    ...
    logger?.debug('browsing.document', { url: workload.pageUrl, ok: cold.ok, totalMs: cold.totalMs });

    if (!cold.ok) {
      return summarize({ workload, resources, error: cold.error || 'cold_failed' });
    }
    ...
    for (const assetUrl of assets) {
      ...
      logger?.debug('browsing.asset', { url: assetUrl, ok: result.ok, totalMs: result.totalMs });
    }
...
function summarize({ workload, resources, error }) {
  ...
  const successRate = resources.length === 0 ? 0 : successes.length / resources.length;
