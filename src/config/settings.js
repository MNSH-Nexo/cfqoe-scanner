...
export function createDefaultSettings(platform = process.platform) {
  ...
  return {
    ...
    mode: 'full',
    scan: {
      perRange: 3, maxCandidates: 120, rounds: 3, screeningRounds: 1,
      concurrency: eligibilityConcurrency, timeoutMs: 6000, minimumSuccessRate: 0.6,
      seed: 404, delayedRetry: { enabled: true, maxCandidates: android ? 30 : 60 },
    },
    verification: {
      enabled: true, limit: android ? 12 : 20,
      sprt: { p0: 0.6, p1: 0.9, alpha: 0.05, beta: 0.1, minRounds: 2, maxRounds: 16 },
    },
    calibration: {
      enabled: true, levels: android ? [1, 2, 4, 6] : [1, 2, 4, 8, 12, 16],
      latencyInflationLimit: 0.1, failureRateIncreaseLimit: 0.02,
    ...
    browsing: {
      ...
      assetLimit: android ? 8 : 10, timeoutMs: 20000, maxSockets: 1,
      estimatedMaxBytes: android ? 3 * MIB : 5 * MIB,
    },
    streaming: {
      enabled: true, workloads: ['apple-advanced-hevc'],
      maxSegments: android ? 3 : 4, quickSegments: 2, researchSegments: 6,
      estimatedMaxBytes: android ? 8 * MIB : 12 * MIB,
      startupBufferSec: 4, safetyFactor: 1.25, timeoutMs: 30000,
      variantMode: 'fixed', targetMbps: 6,
    },
    load: {
      enabled: true,
      durationMs: android ? 10000 : 12000,
      chunkBytes: 1 * MIB,
      maxDownloadBytes: android ? 12 * MIB : 24 * MIB,
      flows: android ? 3 : 6,
      uploadBytes: android ? 1 * MIB : 2 * MIB,
      uploadFlows: 2,
      fanoutRequests: android ? 4 : 6,
      idleSamples: 4, timeoutMs: 20000,
      minBytes: android ? 6 * MIB : 12 * MIB,
      gateProfile: 'balanced',
      endpoints: {
        download: 'https://speed.cloudflare.com/__down', upload: 'https://speed.cloudflare.com/__up',
        ping: 'https://speed.cloudflare.com/__down?bytes=1000', control: 'https://ash-speed.hetzner.com/100MB.bin',
      },
      control: { enabled: false, bytes: 6 * MIB },
      gates: {},
    },
    traffic: {
      targetBytesPerCandidate: android ? 28 * MIB : 44 * MIB,
      hardWarningBytesPerCandidate: 50 * MIB,
    },
    hard: {
...
const UNDERSIZED_V2_VALUES = [
  ['streaming', 'maxSegments', 10], ['streaming', 'quickSegments', 3],
  ['streaming', 'timeoutMs', 25000], ['browsing', 'assetLimit', 6], ['browsing', 'timeoutMs', 15000],
];
const STALE_WORKLOAD_SELECTIONS = [
  ['browsing', ['wikipedia']], ['streaming', ['apple-bipbop']],
];
...
export function migrateSettings(stored, platform = process.platform) {
  ...
  for (const [section, key, staleDefault] of UNDERSIZED_V2_VALUES) {
    if (stored[section] && stored[section][key] === staleDefault) upgraded[section][key] = defaults[section][key];
  }
  for (const [section, staleSelection] of STALE_WORKLOAD_SELECTIONS) {
    if (stored[section] && sameList(stored[section].workloads, staleSelection)) upgraded[section].workloads = defaults[section].workloads;
  }
  ...
  if (storedVersion < 4) upgraded.load = mergeSettings(defaults.load, { gates: storedGates });
    ...
    upgraded.load = mergeSettings(upgraded.load, {
      maxDownloadBytes: defaults.load.maxDownloadBytes,
      durationMs: Math.min(Number(upgraded.load.durationMs) || defaults.load.durationMs, defaults.load.durationMs),
      uploadBytes: Math.min(Number(upgraded.load.uploadBytes) || defaults.load.uploadBytes, defaults.load.uploadBytes),
      fanoutRequests: Math.min(Number(upgraded.load.fanoutRequests) || defaults.load.fanoutRequests, defaults.load.fanoutRequests),
    });
    upgraded.streaming.maxSegments = Math.min(Number(upgraded.streaming.maxSegments) || defaults.streaming.maxSegments, defaults.streaming.maxSegments);
    upgraded.browsing.assetLimit = Math.min(Number(upgraded.browsing.assetLimit) || defaults.browsing.assetLimit, defaults.browsing.assetLimit);
  }
...
export function estimateTrafficBytes(settings) {
  ...
  const downlink = disabled ? 0 : Number(load.maxDownloadBytes) || 24 * MIB;
  ...
  const segments = streaming.enabled === false ? 0 : Number(streaming.estimatedMaxBytes) || 0;
  const transfer = browsing.enabled === false ? 0 : Number(browsing.estimatedMaxBytes) || 0;
