...
// Performs a real WebSocket upgrade against a candidate edge IP.
// Connect time is measured for diagnostics only and never used for ranking.
export function probeWebsocket({ ip, vless, timeoutMs = 6000 }) {
  return new Promise((resolve) => {
    ...
    const onSecureOrConnect = () => {
      ...
      const request = [
        `GET ${vless.path} HTTP/1.1`,
        `Host: ${vless.host}`,
        'Upgrade: websocket',
        'Connection: Upgrade',
        `Sec-WebSocket-Key: ${key}`,
        'Sec-WebSocket-Version: 13',
        'User-Agent: CFQoE/0.5',
        '',
        '',
      ].join('\r\n');
    ...
    try {
      ...
      if (vless.security === 'tls') {
        socket = tls.connect({
          ...
          rejectUnauthorized: !vless.allowInsecure,
