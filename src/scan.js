...
function observedSuccessRate(observations) {
  return observations.length ? observations.filter((item) => item.ok).length / observations.length : 0;
}
...
export async function runScan({ vlessUri, settings, layout, logger, onProgress = () => {}, runId = randomUUID() }) {
  ...
  const candidates = sampleCandidates({
    ranges, perRange: effectiveSettings.scan.perRange,
    max: effectiveSettings.scan.maxCandidates, seed: samplingSeed,
  });
  ...
  const screened = await runInterleaved({
    items: candidates, rounds: screeningRounds,
    concurrency: effectiveSettings.scan.concurrency, seed: samplingSeed,
  ...
  if (effectiveSettings.scan.delayedRetry?.enabled) {
    const retryCandidates = selectDelayedRetries(
      screened.map((entry) => ({ candidate: entry.item, observations: entry.observations })),
      { maxRetries: effectiveSettings.scan.delayedRetry.maxCandidates },
    );
    if (retryCandidates.length) {
      logger.info('scan.delayed_retry.start', { candidates: retryCandidates.length });
      const retried = await runEligibilityBatch({
        candidates: retryCandidates, rounds: 1,
  ...
  const shortlist = candidates
    .map((candidate) => ({ candidate, observations: observationMap.get(candidate.ip) || [] }))
  ...
  if (effectiveSettings.verification?.enabled && shortlist.length) {
    ...
    const verified = await runAdaptiveEligibilityBatch({
      candidates: selected,
      concurrency: Math.min(effectiveSettings.scan.concurrency, selected.length),
      sprt: effectiveSettings.verification.sprt,
      task: (candidate) => probeWebsocket({ ip: candidate.ip, vless, timeoutMs: effectiveSettings.scan.timeoutMs }),
      onCandidateDone: ({ finished, total, decision }) => onProgress({ phase: 'verification', completed: finished, total, note: decision }),
    });
    for (const item of verified) {
      verificationMap.set(item.candidate.ip, item.observations);
      verificationDecision.set(item.candidate.ip, item.decision);
    }
        ...
        for (let round = 1; round <= effectiveSettings.tunnel.rounds; round += 1) {
          ...
          try {
            tunnel = await startXray({
              xrayPath: located.path, vless, candidateIp: candidate.ip,
              startupTimeoutMs: effectiveSettings.tunnel.startupTimeoutMs,
              shutdownGraceMs: effectiveSettings.tunnel.shutdownGraceMs, logger,
            });
            for (const workload of browsingWorkloads) browsing.push(await probeBrowsing({
              workload, proxy: tunnel.socks, timeoutMs: effectiveSettings.browsing.timeoutMs,
              assetLimit: effectiveSettings.browsing.assetLimit, maxSockets: effectiveSettings.browsing.maxSockets, logger,
            }));
            for (const workload of streamingWorkloads) streaming.push(await probeStreaming({
              workload, proxy: tunnel.socks, timeoutMs: effectiveSettings.streaming.timeoutMs,
  ...
  const summaries = candidates.map((candidate) => {
    ...
    summary.selection = {
      ...
      independentlyVerified: Boolean(verification),
      decision: verificationDecision.get(candidate.ip) || 'screened',
    };
    return summary;
  });
  ...
  const written = writeReport({
    ...
    settings: { ...effectiveSettings, tunnel: { ...effectiveSettings.tunnel, xray: xrayInfo.reason || 'ok' } },
    candidates: summaries, startedAt,
  });
  ...
  return {
    ...
    candidateCount: candidates.length, rangeCount: ranges.length,
    samplingSeed, runId,
  };
}
