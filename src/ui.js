...
export const color = {
  ...
  green: (text) => wrap('32', text), yellow: (text) => wrap('33', text), blue: (text) => wrap('36', text), gray: (text) => wrap('90', text),
};
export function banner() {
  return [
    color.blue('+--------------------------------------------+'),
    `${color.blue('|')}      ${color.bold('CFQoE Cloudflare IP Scanner')}  v0.6.0   ${color.blue('|')}`,
    `${color.blue('|')}  ${color.dim('Adaptive measurement with confidence')}       ${color.blue('|')}`,
    color.blue('+--------------------------------------------+'),
  ].join('\n');
}
...
export function terminalWidth() { const columns = Number(process.stdout.columns); return Number.isFinite(columns) && columns > 20 ? columns : 120; }
export function clearScreen() { if (!process.stdout.isTTY) return; readline.cursorTo(process.stdout, 0, 0); readline.clearScreenDown(process.stdout); }
function redrawLine(text) { if (!process.stdout.isTTY) return; readline.cursorTo(process.stdout, 0); readline.clearLine(process.stdout, 0); process.stdout.write(text.slice(0, Math.max(20, terminalWidth() - 1))); }
...
export function clearProgress() { if (!process.stdout.isTTY) return; readline.cursorTo(process.stdout, 0); readline.clearLine(process.stdout, 0); }
