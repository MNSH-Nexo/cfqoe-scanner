...
function candidate(ip, { browsingScore, streamingScore, ok = true }) {
  return buildCandidateSummary({
    ...
    range: '104.16.0.0/13',
    eligibility: [
      { ok, handshakeMs: 120, connectMs: 40, cfRay: '8abc123-FRA' },
      { ok, handshakeMs: 130, connectMs: 45, cfRay: '8abc124-FRA' },
    ],
    tunnel: {
...
test('summaries carry an uncertainty interval and a confidence label', () => {
  const summary = candidate('104.16.0.5', { browsingScore: 80, streamingScore: 80 });
  assert.ok(summary.eligibility.confidence95.lower > 0);
  assert.ok(summary.eligibility.confidence95.upper <= 1);
  assert.equal(summary.eligibility.pops.dominant, 'FRA');
  assert.ok(['none', 'low', 'medium', 'high'].includes(summary.eligibility.confidence));
  assert.ok(summary.scores.conservative <= summary.scores.overall);
  assert.ok(summary.scores.reliabilityLower95 < summary.scores.reliability);
});
...
test('eligibility summaries can be scored later with tunnel data', () => {
  const eligibility = buildEligibilitySummary({
    ip: '104.16.0.8',
    range: '104.16.0.0/13',
    eligibility: [{ ok: true, handshakeMs: 100, connectMs: 30, cfRay: '8abc125-AMS' }],
  });
  const rescored = applyTunnelResults(eligibility, {
    ...
    streaming: [{ score: 88, sustainableMbps: 7, quality: '1080p', startupDelaySec: 1, rebufferRatio: 0, bytes: 1000 }],
  });
  assert.equal(rescored.scores.browsing, 77);
  assert.equal(rescored.scores.streaming, 88);
  assert.equal(rescored.measurement.status, 'complete');
  assert.ok(rescored.scores.overall !== null);
});

test('candidates without tunnel data receive reliability but no overall rank', () => {
  const summary = buildCandidateSummary({
    ip: '104.16.0.2',
    range: '104.16.0.0/13',
    eligibility: [{ ok: true, handshakeMs: 100, connectMs: 30 }, { ok: false, error: 'timeout' }],
    tunnel: null,
    requirements: { browsing: true, streaming: true },
  });
  ...
  assert.equal(summary.measurement.status, 'incomplete');
});

test('rankCandidates puts complete measurements above incomplete ones', () => {
  const ranked = rankCandidates([
    candidate('1.1.1.1', { browsingScore: 40, streamingScore: 40 }),
    candidate('2.2.2.2', { browsingScore: 95, streamingScore: 95 }),
    buildCandidateSummary({ ip: '3.3.3.3', range: '104.16.0.0/13', eligibility: [{ ok: true, handshakeMs: 10, connectMs: 5 }], tunnel: null }),
  ]);
  assert.equal(ranked[0].ip, '2.2.2.2');
  assert.equal(ranked[1].ip, '1.1.1.1');
  assert.equal(ranked[2].ip, '3.3.3.3');
});
...
test('renderTopList produces a tab separated table with confidence', () => {
  ...
  assert.ok(header.includes('Confidence'));
